...
//            add_filter('woocommerce_cart_item_subtotal', array($this, 'cart_item_subtotal'), 10, 3);
     ...
     */
    private function update_deposit_meta($product, $quantity, &$cart_item_data ,$cart_item_key)
    {
            ...
            if ($deposit_enabled && isset($cart_item_data['deposit']) &&
                ...
                if ($product_type === 'variation') {
                    ...
                    if ($override) {
                        ...
                        $amount_type = $product->get_meta('_wc_deposits_amount_type', true);
                        $deposit = floatval($product->get_meta('_wc_deposits_deposit_amount', true));

                    } else {
                        $parent = wc_get_product($product->get_parent_id());
                        $amount_type = $parent->get_meta('_wc_deposits_amount_type', true);
                        $deposit = floatval($parent->get_meta('_wc_deposits_deposit_amount', true));
                    }
                    ...
                    case 'booking':
                        ...
                        if (class_exists('WC_Booking')) {
                            ...
                            if ($amount_type === 'fixed') {
                                ...
                                $deposit = floatval($deposit) * $quantity;

                            } else {
                                $deposit = $amount * (floatval($deposit) / 100.0);
                            }
                        }
...
//                }
        ...
        */
        ...
        $fee_taxes = version_compare(WC()->version, '3.2.0', '>=') ? $cart->get_fee_tax() : $cart->fee_tax;
         ...
         * Taxes handling.
         */
        if (($woocommerce_prices_include_tax !== 'yes' && !wcdp_checkout_mode()) || wcdp_checkout_mode()) {
